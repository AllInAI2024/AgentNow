...
CREATE TABLE IF NOT EXISTS agent_conversations (
    ...
    requirements_json JSON COMMENT '需求收集结果（JSON）',
    structured_result_json JSON COMMENT '最近一次结构化结果缓存（JSON）',
    messages_json JSON COMMENT '会话消息缓存（JSON数组）',
...
INSERT INTO permissions (parent_id, name, code, type, path, icon, sort)
VALUES 
(0, 'Hermes管理', 'hermes', 1, '/hermes', 'robot', 5);
...
-- 检查是否已存在默认模板（避免重复插入）
SET @existing_default_template_id = (
    SELECT id
...
或者直接描述你的需求，我来帮你规划！',
    ...
    @admin_user_id,
    NOW()
WHERE @existing_default_template_id IS NULL;
...
-- 检查默认模板是否已有首个版本快照
SET @existing_default_template_version_id = (
    SELECT id
    FROM agent_template_versions
    WHERE template_id = @default_template_id AND version_no = 1
    LIMIT 1
);
...
SELECT
    ...
    @admin_user_id,
    NOW()
WHERE @default_template_id IS NOT NULL AND @existing_default_template_version_id IS NULL;

-- ============================================
-- 菜单分组说明（前端双列展示）
-- ============================================
-- Hermes 管理菜单共 10 个二级菜单，建议前端采用双列布局：
--
-- 第一列（监控组 + 能力组 - 6 个）
-- ├── 系统概览
-- ├── Profiles 管理
