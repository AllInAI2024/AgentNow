...
### 3.8 保持 Hermes 的“越用越聪明”（接口层必须支持）

AgentNow 的职责是流程与交付闭环，不应在接口设计上切断 Hermes 的学习性与个性化。

接口层必须遵守以下约束：

1. **Profile 稳定**
   - 员工侧接口不得允许前端传入任意 `profile_name`
   - 后端始终通过当前登录用户反查 `user_agents.hermes_profile`

2. **Session 尽量连续**
   - `conversation_id` 代表一段业务会话；后端应优先复用该会话对应的 Hermes session（如有），而不是每条消息都新起会话
   - 只有在用户明确要求“重置/重新开始/新任务”、或后端判断会话不可恢复时，才创建新的 Hermes session，并将新 session 重新绑定到该 `conversation_id`

3. **流程控不语义控**
   - `action_type` 只用于表达“确认/生成/修改”等流程节点，不应强迫用户使用固定格式输入，也不应通过硬规则取代模型的需求抽取与规划

4. **结构化结果是交付载体，不是智能替代**
   - `structured_result` 用于前端渲染与后端生成文件，不应把完整对话压缩成“一条最终提示词”来替代 Hermes 的对话轨迹与记忆积累
