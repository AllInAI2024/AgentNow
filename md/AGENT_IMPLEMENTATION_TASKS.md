...
补充一条“不能扼杀 Hermes 学习性”的硬规则：

1. AgentNow 只控流程节点（下一步做什么），不控语义理解与内容生成（怎么理解、怎么写）
2. 同一员工必须稳定使用同一个 Hermes Profile
3. 同一条业务流程应尽量复用同一 Hermes session 继续推进，避免每轮都新开导致反复补问与无法形成偏好
4. 结构化结果（如 `ppt_content`）用于交付与渲染，不用硬规则取代模型对自然语言的理解
